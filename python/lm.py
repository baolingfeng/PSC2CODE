...
with open(os.path.join(code_dir, "java-lines.json")) as fin:
    JAVA_LINE_STRUCTURE = json.load(fin)

with open(os.path.join(code_dir, "java-words.json")) as fin:
    JAVA_WORDS = json.load(fin)

with open(os.path.join(code_dir, "words")) as fin:
