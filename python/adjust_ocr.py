...
class GoogleOCRParser:
    def __init__(self, video_name, ocr_folder):
        self.video_name = video_name
        self.ocr_folder = ocr_folder
        self.init_from_folder(ocr_folder)
    ...
    def init_from_folder(self, ocr_folder):
        ...
        self.audios, self.audio_word_set = translate_srt_script(
            "../Audios/%s.srt" % self.video_name)
    ...
    def get_audio_text(self, frame):
        # print 'get audio text', frame
        found = -1
        for i, (start, end, text) in enumerate(self.audios):
            if frame >= start and frame <= end:
                found = i
                break

        # print found
        return self.audios[found][2] if found >= 0 else ''
...
def main():
    # video_name = "Intermediate Java Tutorial - 2 - Some More String Methods"
    # video_name = "Intermediate Java Tutorial - 4 - Introduction to Collections"
    # video_name = "Intermediate Java Tutorial - 6 - LinkedList"
    # video_name = "Java Tutorial For Beginners 3 - Creating First Java Project in Eclipse IDE"
    # video_hash = "U8wrZRYAnmI"
    # target = ("Java Tutorial For Beginners 3 - Creating First Java Project in Eclipse IDE", "U8wrZRYAnmI")
    # target = ("Java Programming Tutorial - 4 - Hello YouTube", "SHIT5VkNrCg")
    # target = ("Java Programming Tutorial - 52 - Event Handling", "3EE7E3bvfe8")
    # target = ("Java Tutorial For Beginners 15 - Java String", "sliTbMkQBZ4")
    # target = ("Java Tutorial For Beginners 32 - LinkedList in Java", "EWr6rFqJUTU")
    # target = ("Java Programming Tutorial - 17 - Constructors", "tPFuVRbUTwA")
    # target = ("Java Programming Tutorial - 36 - Time Class", "o4Or0PMI_aI")
    # target = ("Intermediate Java Tutorial - 14 - Stacks push pop", "8TMBjfS8wY0")
    # cluster = 0
    ...
    for r in db.querymany(sql):
        ...
        # if target[0] == video_name and target[1] == video_hash:
        # if video_name.find("Intermediate Java Tutorial") >= 0:
        # if playlist == "PLS1QulWo1RIbfTjQvTdj8Y6yyq4R7g-Al":
        # if video_hash == 'kj6mYRrJeU8':
        # if os.path.exists(ocr_folder):
        # if not os.path.exists(os.path.join(ocr_folder, "parse")):
        if video_hash in process_hashes:
            ...
            #     if frame == 524:
            #         # print doc['lines']
            #         print generate_doc(doc['lines'])

        # break
            # parser.correct_words()
            # parser.cluster_docs()
            # parser.generate_actions()
            # for idx, doc in enumerate(parser.docs):
            #     if idx > len(parser.docs) - 1:
            #         continue

            #     next_doc = parser.docs[idx+1]
            #     sim, lcs_lines, deleted, inserted, delta = diff_lines(doc['lines'], next_doc['lines'])
            ...
            # break
            # if len(parser.docs) > 0:
            #     parser.correct_words()
            # parser.cluster_lines()

    # parser = GoogleOCRParser(target[0], ocr_folder)
    # parser.adjust_docs()
    # parser.cluster_docs()
    # parser.cluster_lines()
    # print read_Google_Vision_result("%s/478_0.json" % ocr_folder)
